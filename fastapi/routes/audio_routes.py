...
from controller.audioController import get_audios_by_process_id_db, get_audioFile, create_audio_db, delete_audio, update_audio_title_db, update_specialist_analysis_db
...
@router.put("/audioAnalysis/{audio_id}")
async def update_specialist_analysis(audio_id: int, analysis: str, db: Session = Depends(get_db)):
    return update_specialist_analysis_db(audio_id, analysis, db)
